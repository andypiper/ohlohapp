...
#
http = Net::HTTP.start('www.ohloh.net', 80) do |session|

	# pass the MD5 hash of the email address
  request = Net::HTTP::Get.new("/accounts/#{Digest::MD5.hexdigest(email)}.xml?&api_key=#{api_key}")
